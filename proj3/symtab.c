...
void insert(char *name, Type_Expression type, int offset, int rowSize) {
  int currentIndex;
  int visitedSlots = 0;
  ...
  HashTable[currentIndex]->rowSize = rowSize; /* in int (4 bytes) units */
}
...
void PrintSymbolTable() {
  ...
  for (i=0; i < HASH_TABLE_SIZE; i++) {
    if (HashTable[i] != NULL) {
      switch (HashTable[i]->rowSize) {
      case -1 : printf("\tSCALAR \"%s\" of type %s with offset %d\n", 
		HashTable[i]->name, TypeToString(HashTable[i]->type), HashTable[i]->offset); 
	        break;
      case  0 : printf("\t1-DIM ARRAY \"%s\" of element type %s with offset %d\n", 
		HashTable[i]->name, TypeToString(HashTable[i]->type), HashTable[i]->offset); 
	        break;
      default:  printf("\t2-DIM ARRAY \"%s\" is not supported\n", 
		       HashTable[i]->name);
      }
    }
  }
